...
The debt balance is reduced by redemptions from the Transmuter. Each redemption reduces debt and the underlying collateral by the same value. Yield generated by the MYT accrues to the collateral and offsets that draw. Maintaining a long-term position therefore calls for a conservative LTV, where yield generation is sufficient to offset the impact of the redemption rate.
...
| Vault yield and redemptions accrue | Redemptions gradually retire debt and remove matching collateral. Yield replenishes the collateral. If the yield on the collateral outpaces what redemptions draw (a function of LTV and the redemption rate), the collateral keeps growing while the debt falls to zero. |
...
Strategy designers can propose new strategy adapters for inclusion in the MYT basket. After an independent audit and risk classification, the Curator can add the adapter and set its caps. Well-performing strategies improve the vault’s risk-adjusted return. Compensation for strategy designers is arranged with the DAO; the vault contracts themselves pay only the protocol’s performance fee.
